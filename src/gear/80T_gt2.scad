...
// MODS by COSMOS 2015
// Wades biggear modification by mblaster 09/2015
...
// tuneable constants
// Para poleas de 16T son 90T para un L3K (COSMOS)
// Para poleas de 20T son 86T para un L3K
teeth = 86;			// Number of teeth, standard Mendel T5 belt = 8, gives Outside Diameter of 11.88mm
...
motor_shaft = 9;	// NEMA17 motor shaft exact diameter = 5
...
num_holes=6;
hole_size=7;
hole_offset=17; // 18 para 90T, 17 para 86T
...
difference () {
    ...
    if ( wades_biggear == 1 ) { nut_trap( nut_trap_thickness, m8_nut_diameter, nut_trap_depth ); }
    if ( teardrop_holes == 1) { teardropholes ( num_holes, hole_offset, hole_size, nut_trap_depth ); }
    }
//
// Functions
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	difference()
	 {	 
		...
		if ( pulley_b_ht < m3_nut_flats ) {} else {
		if ( (pulley_b_dia - motor_shaft)/2 < m3_nut_depth + 3 ) {} else {
...
// Nut trap to use as big gear on a wades extruder
module nut_trap(nut_trap_thickness, m8_nut_diameter, nut_trap_depth) {
...
// Teardrop holes
module teardropholes ( num_holes, hole_offset, hole_size, depth ) {
    for (hole=[0:num_holes-1])
    {
        ...
        translate([0,0,-3])
        {
            cylinder(r=hole_size,h=depth+2);
            //cube([hole_size,hole_size,depth+2]);
        }
    }
    // Poner texto
    //DrawTextH(ELLO,11);
    //translate([0,0,11]) cylinder(r=24,h=15,$fn=100);

}

module DrawLetter(letter, heigh, size) 
{
    linear_extrude(heigh, center = true, convexity = 0, twist = 0)
    rotate([0,0,180])
    translate([-size/2,0,0])
        text(letter,size,,true,font="Arial Rounded MT Bold");
}

module DrawTextH(Texto,Radio) 
 {
    nArco=360/len(Texto);
    for(i=[0:len(Texto)-1])
        {
        rotate([0,0,-i*nArco])
            translate([0,-Radio,0])
                DrawLetter(str(Texto[i][0]),24,13);
        }
 }
